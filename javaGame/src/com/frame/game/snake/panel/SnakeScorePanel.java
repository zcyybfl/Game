 ...
 * @version 1.0
 */
public class SnakeScorePanel extends JPanel {

    private JTable table;
    ScoreTable model = new ScoreTable();
    UserDao userDao = new UserDao();

    public SnakeScorePanel() {
        this.setLayout(null);
        init();
    }
    ...
    public void init() {
        ...
        for (int i = 0;i < list.size() - 1;i++) {
            for (int j = 0;j < list.size() - 1 - i;j++){
                if (user[j].getSnakeScore() < user[j + 1].getSnakeScore()) {
                    user1 = user[j];
                    user[j] = user[j + 1];
                    user[j + 1] = user1;
                }
        ...
        model.setRowCount(0);
        for (int i = 0; i < list.size();i++) {
            model.addRow(new Object[]{i + 1,user[i].getUsername(),user[i].getSnakeScore()});
